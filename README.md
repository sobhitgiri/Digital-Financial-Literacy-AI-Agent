💬 Digital Financial Literacy AI Agent

An AI-powered assistant designed to improve digital financial literacy using IBM Watsonx.ai and IBM Granite foundation models. This chatbot helps users understand UPI, budgeting, scams, personal loans, and investing—offering reliable, document-grounded, and multilingual financial guidance.

<img width="1024" height="1024" alt="Digital Financial Literacy AI Agent" src="https://github.com/user-attachments/assets/7ac3c605-b9a8-4d9b-877d-d2186e190937" />

🧩 Problem Statement

Many individuals, especially in underserved regions, lack awareness of digital financial tools and safe practices. Without trusted guidance, they may fall victim to scams or mismanage personal finance. This project aims to bridge that gap with an AI-powered solution.

💡 Proposed Solution

A document-grounded AI chatbot trained using IBM Watsonx.ai and IBM Granite, offering multilingual and easy-to-understand answers on financial topics like UPI, budgeting, scams, loans, and investment basics.

🧠 Technologies Used

- 🧠 IBM Granite Foundation Model (Watsonx.ai)
- 🧰 Retrieval-Augmented Generation (RAG)
- 🗃️ IBM Vector Index from RBI, NPCI, and financial PDFs
- ☁️ IBM Cloud Lite Account
- 🧠 Natural Language Processing (NLP)
- 🧩 IBM Cloud Object Storage

☁️ IBM Cloud Services Used

-Watsonx.ai Studio
-IBM Granite Model
-Watsonx Vector Index
-IBM Cloud Lite Account
-IBM Cloud IAM
-IBM Cloud Object Storage

👥 Target Users

- General public seeking finance clarity
- Rural/semi-urban citizens & students
- NGOs, outreach programs, SHGs
- First-time UPI and digital banking users

🌟 WOW Factors
  
 - Uses RAG to fetch answers from real RBI/NPCI or uploaded PDFs
 -Built entirely on IBM Cloud using Watsonx tools
 -Handles unrelated questions with polite redirection
 -Future-ready with multilingual support
 -Educates users on scams, interest rates, budgeting, stock market, UPI, and more
 -Provides safe and personalized financial guidance


🌟 Key Features

- ✅ IBM Watsonx AI-powered Q&A
- 📚 Document-grounded answers (RBI/NPCI PDFs)
- 🔐 Scam and fraud awareness
- 📊 Budgeting, loans, and stock investing guidance
- 💬 Multilingual-ready with polite redirection for off-topic queries

🚀 How It Works

1. Users ask a question (e.g., "What is a phishing scam?")
2. IBM Granite processes the query using NLP
3. Vector Index retrieves relevant content from uploaded PDFs
4. AI returns clear, safe, and informative answers

🖼️ Screenshots

🔹 RESOURCES LIST :-

<img width="1919" height="875" alt="Resource List" src="https://github.com/user-attachments/assets/ebfe3141-8261-491b-adb2-4537749f51db" />


🔹 IBM GRANITE USED IN WATSON.AI STUDIO :-

<img width="986" height="204" alt="IBM GRANITE" src="https://github.com/user-attachments/assets/aae229b9-9c0e-4373-ab0c-ab733a2c1ae7" />


🔹 SETUP :-

<img width="1919" height="1004" alt="setup" src="https://github.com/user-attachments/assets/90f6084e-1e38-4f2a-b2f4-c15eacc15946" />


🔹 AGENT INSTRUCTIONS :-

<img width="1918" height="1010" alt="Agent_Instructions" src="https://github.com/user-attachments/assets/58b0c1b7-a204-4f17-8489-3a583e36a194" />


🔹PROMPT QUESTIONS :-

<img width="1917" height="875" alt="Prompt_questions" src="https://github.com/user-attachments/assets/8b9cde70-d055-49c8-9903-c8f1b2290b6a" />


🔹TOOLS USED :-

<img width="1919" height="1002" alt="Model tools_testing2" src="https://github.com/user-attachments/assets/1c7abe6d-5ea7-4966-b1f5-be618098d9d6" />
<img width="1919" height="1002" alt="Model tools_testing" src="https://github.com/user-attachments/assets/fe0d6c0e-5c47-4cf4-9e6e-105a049705a4" />


🔹 TESTING :-

<img width="1919" height="875" alt="Testing4" src="https://github.com/user-attachments/assets/1960ef3c-a615-4fbf-910d-e80ec4797b77" />
<img width="1919" height="879" alt="Testing3" src="https://github.com/user-attachments/assets/30cf26b2-26f4-4d35-b659-aac40e208852" />
<img width="1909" height="880" alt="Testing2" src="https://github.com/user-attachments/assets/fe0ea564-fffb-4617-a337-ff30ecad92a5" />
<img width="1918" height="868" alt="Testing1" src="https://github.com/user-attachments/assets/ca94ce44-2568-4e6f-b920-9cd55385f7d0" />


🔹 DEPLOYMENT :-

<img width="1919" height="1009" alt="Deployment" src="https://github.com/user-attachments/assets/b99987f6-d13e-44f4-bf45-c608ed1a31d2" />


 🔹 API KEY REFERENCES AFTER DEPLOYMENT :-
 
 <img width="1908" height="1006" alt="Links" src="https://github.com/user-attachments/assets/f1515c45-0669-4525-baa1-85cb56764661" />
 

 🔹 CODE SNIPPET :-
 
 <img width="1919" height="1004" alt="Screenshot 2025-08-03 101001" src="https://github.com/user-attachments/assets/9c00c8d1-51d0-41b1-a3e2-4329fa71b66e" />
 
 
📌 How to Run or Deploy

-Log in to IBM Cloud Lite: https://cloud.ibm.com
-Launch Watsonx.ai Studio
-Create a new AI Agent
-Upload financial PDFs to a Vector Index
-Choose Tools for web search (Google,Wikipedia,DuckDuckGo etc..)
-Configure agent instructions and topics (restricting AI from answering off-topic questions politely)
-Test in the preview panel
-Deploy via web snippet, Streamlit, or custom web UI

🛣️ Future Scope

- Voice assistant and WhatsApp bot
- Regional language expansion via Watson Language Translator
- Monthly financial summary reports
- OCR support for image-based inputs

📌 Useful Links

- [IBM Watsonx.ai](https://www.ibm.com/products/watsonx-ai)
- [IBM Granite Models](https://www.ibm.com/blog/announcing-granite)
- [IBM Cloud Lite](https://cloud.ibm.com)
- [RBI Website](https://rbi.org.in)
- [NPCI FAQs](https://www.npci.org.in)

⚖️ License

This project is licensed under the MIT License.

 🙌 Credits

Created by [Sobhit Giri](https://github.com/sobhitgiri)  
As part of the IBM SkillsBuild for Academia Internship 2025  
Powered by IBM Cloud + Watsonx + Granite
